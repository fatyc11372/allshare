...
interface AddItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (item: any) => void; // 👈 關鍵修正：改回 onSubmit
  currentUser: any;
}
...
export function AddItemModal({ isOpen, onClose, onSubmit, currentUser }: AddItemModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit(newItem); // 👈 關鍵修正：改回 onSubmit
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-6">
          <div className="border-2 border-dashed border-muted-foreground/30 rounded-2xl aspect-[16/10] bg-muted/20 flex flex-col items-center justify-center relative overflow-hidden group">
            {formData.image ? (
              <>
                ...
                <Button size="icon" variant="destructive" className="absolute top-2 right-2 h-7 w-7 rounded-full opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => setFormData({ ...formData, image: '' })}>
                  <X className="h-4 w-4" />
                </Button>
              </>
            ) : (
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-5">
            ...
            <div className="flex items-center gap-3 col-span-2 bg-muted/30 p-4 rounded-xl border">
              ...
              <Input type="date" value={formData.availableFrom} onChange={(e) => setFormData({ ...formData, availableFrom: e.target.value })} className="bg-transparent border-none p-0 focus-visible:ring-0" required />
              <span className="text-muted-foreground">至</span>
              <Input type="date" value={formData.availableTo} onChange={(e) => setFormData({ ...formData, availableTo: e.target.value })} className="bg-transparent border-none p-0 focus-visible:ring-0" required />
            </div>
